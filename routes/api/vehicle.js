...
router.post("/openOne", async (req, res) => {
  let id = req.body.id;
  console.log("vehicle-openOne", id);

  try {
    let vehicle = await Vehicle.find({ id });
    console.log(vehicle);
    res.status(200).send({status: vehicle.length > 0 ? "success" : "noupload", vehicle});
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server error");
  }
...
router.post("/openLanding", async (req, res) => {
  ...
  console.log("openLanding", req.body);
  try {
    const totalDocuments = await Vehicle.find();
    total = totalDocuments.length;
    ...
    console.log("total, searchResult.length", total, searchResult.length);
    res.status(200).send({ data: searchResult, total, count: searchResult.length });
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server error");
  }
